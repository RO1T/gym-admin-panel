<template>
                  ...
                  <div class="video__time">
                    ...
                    <div class="body__video_buttons">
                      <q-btn
                        ...
                        @click="
                          onEdit(getVideoNameById(schedule.VideoID), schedule.VideoID, day.order);
  ...
  <q-dialog v-model="isEditModalOpen">
    ...
    >
      ...
      <q-time v-model="timeWithSeconds" landscape @update:model-value="onSelectedTimeChange" />
      <div style="width: 80%; display: flex; gap: 20px; margin-bottom: 20px">
        <q-btn label="Сохранить" color="positive" style="width: 50%" @click="onSave" />
        <q-btn label="Отмена" color="negative" style="width: 50%" @click="onDecline" />
      </div>
    </q-card>
  </q-dialog>
</template>
...
const init = () => {
  days.value.forEach(async (day, idx) => {
    setTimeout(async () => {
      const response = await api.get(`/schedule/${idx}`,{
        headers: {
          Authorization: `Bearer ${token}`
        }
      })
      if(response.status === 200){
         day.items = response.data
      }
    }, idx * 50)
  })
}
...
const _removeSchedule = async () => {
  const response = await api.delete(`/schedule/${index.value}`,{
     headers: {
      Authorization: `Bearer ${token}`,
    }
  })
...
const onSelectedTimeChange = (value: string | null) => {
  if (value) videoToSchedule.value.time = value
}
